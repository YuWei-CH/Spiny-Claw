...
- [`scripts/init_cuda_toolchain.sh`](/home/yuwei/Documents/Spiny-Claw/scripts/init_cuda_toolchain.sh)
  Installs the CUDA toolkit when needed and then verifies that `nvcc`, CUDA headers, and CUDA development libraries are available locally for future `local_compile` use.
...
2. Run [`scripts/init_cuda_toolchain.sh`](/home/yuwei/Documents/Spiny-Claw/scripts/init_cuda_toolchain.sh) to install and verify the local CUDA toolchain for compile prechecks
...
- CUDA toolkit installation and verification are treated as one-time machine setup, not as part of the `local_compile` skill itself
